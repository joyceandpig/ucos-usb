...
void usb_port_set(u8 enable)
{
	...
	if(enable)_SetCNTR(_GetCNTR()&(~(1<<1)));//退出断电模式    连接usb 控制寄存器的第二位置0
	else                                                      // 断开usb
	{	  
		_SetCNTR(_GetCNTR()|(1<<1));  // 断电模式  控制寄存器的第二位置1
		GPIOA->CRH&=0XFFF00FFF;
		GPIOA->CRH|=0X00033000;
		PAout(12)=0;	    		  //D+为低电平，表示断开usb连接
	}
}
...
//void Joystick_Send(u8 buf0,u8 buf1,u8 buf2,u8 buf3, u8 buf4, u8 buf5, u8 buf6, u8 buf7)
{
	...
	Mouse_Buffer[0]=buf0;//判断哪个按键按下了，3：中键  0x01：左键 0x02：右键 
	Mouse_Buffer[1]=buf1;//鼠标滑动的x方向的距离，相对偏移量，向右为正 -127-127
	Mouse_Buffer[2]=buf2;//鼠标滑动的y方向的距离，相对偏移量，向下为正  -127-127
	Mouse_Buffer[3]=buf3;// 鼠标滚轮的偏移量（敏感度），左右键都为0 鼠标滑动也为0  只有滚轮移动时存在偏移量，向上为正
												//-127-127
