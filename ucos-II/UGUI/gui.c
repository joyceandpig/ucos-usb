...
void gui_init(void)
{
	...
	gui_phy.language = 0;//gui显示字体语言类型
}   
//获取键值参数
...
void gui_show_strmid(u16 x,u16 y,u16 width,u16 height,u16 color,u8 size,u8 *str)
{
	...
	if(height>size)yoff=(height-size)/2;//除2是为了将起始位置放在中间位置
...
void gui_show_ptchar(u16 x,u16 y,u16 xend,u16 yend,u8 offset,u16 color,u16 size,u8 chr,u8 mode)
{
	...
	{		   
	    ...
	    {   
			...
			else temp = asc2_2412[chr][pos];  //调用2412字体
			tempoff=offset;
			if(x>xend)return;						//超区域了 	                          
	    for(t=0;t<8;t++)//对一列像素点进行显示，直到显示完一列在跳转至第二列
			{			    
		    if(tempoff==0)
				{
					if(temp&0x80)
...
//	printf("%s",dzk);
	...
	if(mode==0) 	//非叠加方式
	{
		for(pos=0;pos<csize;pos++)//size*2
		{	
	...
	{
		for(pos=0;pos<csize;pos++)//size*2  横向的x轴移动，比单字符多了宽度
		{	
			if(x>xend)break;//到达终点坐标
		    temp=dzk[pos];	//得到点阵数据                          
			if(tempoff==0)	//偏移地址到了
