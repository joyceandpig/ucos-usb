...
//********************************************************************************
//V1.1 20140318
//_filelistbox_obj结构体新增磁盘label选项，可以显示磁盘名字
//////////////////////////////////////////////////////////////////////////////////

//默认的磁盘名字
const u8 *FLIST_DISK_DNAME_TBL[FLBOX_DISK_NUM]={"SD Card","FLASH Disk"}; 
...
{
	u8 i=0,res=0;
	u8 tbuf[FLBOX_DISK_LABEL_LENTH];
	u32 sn;
	u8 pathbuf[3];
	...
	for(i=0;i<FLBOX_DISK_NUM;i++)							//为LABEL ，申请内存
	{
		filelistbox_crt->disklabel[i]=gui_memin_malloc(FLBOX_DISK_LABEL_LENTH);
		if(filelistbox_crt->disklabel[i]==NULL)
		{
			i=0XFF;
			break;
		}
		gui_memset(filelistbox_crt->disklabel[i],0,FLBOX_DISK_LABEL_LENTH);//清零
		sprintf((char*)pathbuf,"%d:",i);
		res=f_getlabel ((TCHAR *)pathbuf,(TCHAR *)tbuf,(DWORD*)&sn);
		if(res==FR_OK&&strlen((const char *)tbuf))//读取成功，且磁盘名字不为空。
		{
			strcpy((char *)filelistbox_crt->disklabel[i],(const char *)tbuf);  
		}else //读取失败/磁盘盘符为空的时候，采用默认名字 
		{
			strcpy((char *)filelistbox_crt->disklabel[i],(const char *)FLIST_DISK_DNAME_TBL[i]); 
		} 
			
	} 
	if(i==0XFF)//申请失败了
	{
		filelistbox_delete(filelistbox_crt);
		filelistbox_crt=NULL; 
	} 
...
void filelistbox_delete(_filelistbox_obj *filelistbox_del)
{
	...
	for(i=0;i<FLBOX_DISK_NUM;i++)gui_memin_free(filelistbox_del->disklabel[i]);//删除label占用内存 
...
u8 filelistbox_scan_filelist(_filelistbox_obj *filelistbox)
{
	...
	}else//没有任何有效路径,说明回到选择磁盘的状态了
	{
		...
		filelistbox->foldercnt=0;
		filelistbox->filecnt=0;
 		filelistbox_addlist(filelistbox,filelistbox->disklabel[0],0);		//磁盘0
		filelistbox_addlist(filelistbox,filelistbox->disklabel[1],0);		//磁盘1
 	}
...
void filelistbox_check_filelist(_filelistbox_obj *filelistbox)
{
	...
	{   
		...
		{	
			...
			if(filelistbox->path)gui_memin_free(filelistbox->path);//释放之前申请的内存
